...
from src import model as quickdraw_model
import cv2
import numpy as np
import torch
...
def get_args():
    ...
    parser.add_argument(
        "-c", "--color", type=str, choices=["green", "blue", "red"], default="green",
        help="Color which could be captured by camera and seen as pointer"
    )
    parser.add_argument(
        "-a", "--area", type=int, default=3000, help="Minimum area of captured object"
    )
    parser.add_argument(
        "-d", "--display", type=int, default=3, help="How long is prediction shown in second(s)"
    )
    parser.add_argument(
        "-s", "--canvas", type=bool, default=False, help="Display black & white canvas"
    )
...
def main(opt):
    ...
    # Load the video from camera (built-in webcam)
    ...
    if torch.cuda.is_available():
        ...
        torch.serialization.add_safe_globals([quickdraw_model.QuickDraw])
        model = torch.load(
            "trained_models/whole_model_quickdraw",
            map_location="cpu",
            weights_only=False
        )
    model.eval()
    ...
    while True:
        ...
        if not is_drawing and not is_shown:
            if len(points):
                ...
                # Corrected findContours for OpenCV 4.x
